...
test("successful handoff persists the reducer result before publishing", async () => {
  ...
  const board = createTask({ id, workflow: "quick", prefix: "qk", title: "success", description: "d", resolved });
  board.phase = "QA";
  board.owner = "qa";
  board.sessions = {
  ...
  const result: any = await tools.handoff.execute("call", {
    to: "worker", phase: "IMPLEMENTING", message: "implement now", verdict: "PASS",
  });
