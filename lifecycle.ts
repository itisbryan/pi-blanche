...
export function registerLifecycle(pi:any,deps:Deps):void {
  ...
  pi.registerTool?.({name:"checkpoint",description:"Persist a crew checkpoint.",parameters:{},execute:async(_id:string,input:CheckpointInput)=>{const b=current(pi),role=process.env.BLANCHE_ROLE as Role;if(!role)throw Error("Role is required");const s=b.sessions[role];const path=writeCheckpoint(b,role,b.currentSpec,s?.contextEpoch??0,input);if(s)s.latestCheckpoint=`checkpoints/${path.split('/').pop()}`;writeBoard(b);return {content:[{type:"text",text:path}]};}});
  pi.registerTool?.({name:"consult",description:"Request a consultation.",parameters:{},execute:async(_id:string,input:any)=>{const b=current(pi),from=process.env.BLANCHE_ROLE as Role,role=input.role as Role;if(role!=="researcher"&&role!=="advisor")throw Error("Consult role must be researcher or advisor");const id=crypto.randomUUID(),path=`consultations/c-${id}.md`;const rec={id,role,requestedBy:from,spec:b.currentSpec,reworkRound:b.currentSpec?b.specs[b.currentSpec]?.reworkRound??0:b.reworkRound,summaryPath:path};deps.channel()?.publish({type:"consult",taskId:b.id,consultationId:id,role,question:input.question,context:input.context});const file=writeConsultation(b,rec,input.answer??"");rec.summaryPath=file;b.consultations.push(rec); if(b.currentSpec&&b.specs[b.currentSpec])b.specs[b.currentSpec].lastAdvisorConsultedRound=rec.reworkRound;else b.lastAdvisorConsultedRound=rec.reworkRound;writeBoard(b);return {content:[{type:"text",text:file}]};}});
}
