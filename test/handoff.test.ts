...
function mkBoard(over: any = {}): any {
  return {
    id: "t-1", workflow: "feat", prefix: "mb", cwd: "/tmp",
    status: "active", phase: "IMPLEMENTING", owner: "worker", revision: 0,
    task: { title: "t", descriptionPath: "task.md" },
    specs: {},
    consultations: [],
    leader: { sessionName: "leader-session" },
    resolved: {
      workflow: "feat", prefix: "mb", roster: ["planner", "advisor", "worker", "qa", "verifier"],
      agents: {}, phases, specs: false, advisorAfter: null, maxRework: 3, maxWorkers: 1,
      configRevision: "sha",
    },
    sessions: {
    ...
    reworkRound: 0, lastAdvisorConsultedRound: null,
    history: [],
    ...over,
  };
}
...
const call = ({ board: patch = {}, ...rest }: any) =>
  decideHandoff({ board: mkBoard(patch), from: "qa", to: "worker", phase: "IMPLEMENTING",
    liveSessions: live, now: 123, handoffId: "h-1", ...rest } as any);
...
test("qa PASS to worker moves phase, owner and appends history", () => {
  ...
  assert.deepEqual(
    { ...r.board.history[0] },
    { handoffId: "h-1", from: "qa", to: "worker", spec: undefined, phase: "QA", verdict: "PASS", sentAt: 123 },
  );
...
test("a rejected decision returns the board deep-equal to the input", () => {
  ...
  decideHandoff({ board: before, from: "qa", to: "researcher", phase: "QA", verdict: "PASS",
    liveSessions: live, now: 1, handoffId: "h" } as any);
...
test("rework is per spec when specs are on: s02 failing twice leaves s03 at 0", () => {
  const withSpecs = {
    ...
    specs: {
      s02: { status: "implementing", path: "p", dependsOn: [], reworkRound: 1, lastAdvisorConsultedRound: null },
...
test("advisorAfter null never nudges", () => {
  const r = call({ from: "qa", verdict: "FAIL", board: { reworkRound: 5, resolved: { ...mkBoard().resolved, advisorAfter: null, maxRework: 99 } } });
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.notes.some((n) => /[Aa]dvisor/.test(n)), false);
});
...
test("advisorAfter 2 nudges at rework 2 and again at 3 until a consultation lands", () => {
  ...
  if (at2.ok) assert.equal(at2.notes.some((n) => /[Aa]dvisor/.test(n)), true, "fires at 2");

  const at3 = call({ from: "qa", verdict: "FAIL", board: { reworkRound: 2, resolved: res } });
  assert.equal(at3.ok, true);
  if (at3.ok) assert.equal(at3.notes.some((n) => /[Aa]dvisor/.test(n)), true, "fires again at 3, not once");

  const consulted = call({ from: "qa", verdict: "FAIL", board: { reworkRound: 2, lastAdvisorConsultedRound: 3, resolved: res } });
  assert.equal(consulted.ok, true);
  if (consulted.ok) assert.equal(consulted.notes.some((n) => /[Aa]dvisor/.test(n)), false, "silent once consulted for that round");
});

test("a missing checkpoint warns but does not block", () => {
  const r = call({
    from: "qa", verdict: "PASS", phase: "QA",
    board: { sessions: { ...mkBoard().sessions, qa: { sessionName: "q", contextEpoch: 4 } } },
  });
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.notes.some((n) => /no checkpoint/i.test(n)), true);
});
