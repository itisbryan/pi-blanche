...
test("resume, stop, and clean are reachable through the one /crew command", async () => {
	...
	const channel = {
		listSessions: async () => [{ name: `${resumeId}-worker` }, { name: `${resumeId}-qa` }],
		publish: () => undefined,
	};
	const oldHerdr = process.env.HERDR_BIN;
  ...
  `,
	...
	const run = (mode: string) =>
		new Promise<void>((resolve, reject) => {
			const child = spawn(process.execPath, [cli, childPath, id, mode], {
	...
	const after = readBoard(id);
	assert.equal(
		after.history.some((entry) => entry.handoffId === "concurrent-handoff"),
		true,
	);
